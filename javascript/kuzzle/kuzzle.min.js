...
// as <script src="path/to/kuzzle/kuzzle.js"></script>
  ...
  if(typeof io == "undefined"){
    ...
    // as Node.js common.js module
      ...
      console.log("There is something wrong with socketio dependency");
    }
  }
  ...
  var Kuzzle = {
    socket : null,
    options: {
      autocommit: false,        // should we make aotocommits ? (commit automatically when autocommitLimit number of bulk requests are reached)
      autocommitLimit: 100000,  // make a silent commit each autocommitLimit records
      commitBeforeClose: true  // should we commit before kuzzle close, so that we do not loose data ?
    },
    subscribedRooms: [],
    bucket: {},                 // the bulk request bucket
    currentBulkOperations: {},  // stores the current operation type (create, delete...) for each collections

    init: function (socketUrl, options) {
   ...
   /**
     ...
     * @param {String} collection
     * @param {Object} data
     * @param {String} action to perform (example: 'search')
     * @param {Function} callback
     */
   ...
   /**
     ...
     * @param {String} action
     * @param {Object} data
     * @return {Integer} the current collection queue length
     */

    prepare: function(collection, action, data, callback) {
      var bag = []
        , i
        , bucketLength;
      if (!this.bucket[collection]) {
        this.bucket[collection] = [];
        this.currentBulkOperations[collection] = null;
      }
      switch(action) {
        case 'write':
        case 'insert':
        case 'create':
          if (this.currentBulkOperations[collection] != 'create') {
            bag.push({create: {}});
            this.currentBulkOperations[collection] = 'create';
          }
          bag.push(data);
          break;
        case 'update':
          if (this.currentBulkOperations[collection] != 'update') {
            bag.push({update: {_retry_on_conflict: 3}});
            this.currentBulkOperations[collection] = 'update';
          }
          bag.push(data);
          break;
         case 'delete':
          bag.push({delete: {_id: data}});
          this.currentBulkOperations[collection] = 'delete';
          break;
      }
      for (i in bag) {
        this.bucket[collection].push(bag[i]);
      }
      if (callback) {
        callback({autocommit: false, response:{bulkBody: bag}});
      }

      bucketLength = this.bucket[collection].length;

      // autocommit
      if (this.options.autocommit && (this.bucket[collection].length % this.options.autocommitLimit) == 0)  {
        this.commit(collection, callback, true);
      }
    ...
    commit: function(collection, callback, isAutocommit) {
      var committedCollection = 'committed-' + collection;

      if (this.bucket[collection]) {

        this.bucket[committedCollection] = this.bucket[collection];
        delete this.bucket[collection];
        
        this.bulk(collection, this.bucket[committedCollection], function(error, response) {
          if (error) {
            console.log('COMMIT ERROR', error);
          }

          response.autocommit = (isAutocommit === true);
          response.bulkBody = this.bucket[committedCollection];
          delete this.bucket[committedCollection];
        ...
        if (callback) {
          // do not raise an error because, don't having something to commit, ia not really an error...
          callback(null, {status: 'no prepared query for "' + collection + '" collection'});
        }
    ...
    close: function(callback){
      if (this.options.commitBeforeClose) {
        for (var collection in this.bucket) {
          if (this.bucket[collection].length) {
            this.commit(collection, function(response) {
              if (callback) {
                callback(response);
              }
            });
